...
def create_mixer_player(name):
    sub_players = [
        QLearningPlayer("QLearning", learning_rate=0.2, temperature=1),
        QLearningPlayer("QLearning", learning_rate=0.2, temperature=1),
        QLearningPlayer("QLearning", learning_rate=0.2, temperature=0.5),
    ]

    return MixerPlayer(name, players=sub_players, learning_rate=0.2, temperature=1)


def play_game(total_rounds):
    p1 = create_mixer_player("Mixer1")
    p2 = SophisticPlayer("Sophistic")

    mediator = SequenceMediator(num_players=2)
